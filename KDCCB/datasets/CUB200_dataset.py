...
def build_CUB200loader(cfg, training, is_test):
...
class CUB200Dataset(Dataset):
    def __init__(self, 
        ...
        self.split = split
        self.imageset_dir = os.path.join(self.root_dir, 'images')

        if self.split == 'train':
            self.split_dir = os.path.join(self.root_dir, 'labels', 'train.txt')
        if self.split == 'val':
            self.split_dir = os.path.join(self.root_dir, 'labels', 'test.txt')
        ## NOUSE PLACEHOLDER
        if self.split == 'test':
            self.split_dir = os.path.join(self.root_dir, 'labels', 'test.txt')
    ...
    def get_img_label_list(self, imglist):
        ...
        for i in range(len(imglist)):
            cls_name, imgname = imglist[i].split('/')
            cls_name_id, cls_name_str = cls_name.split('.')
            imglb_list.append((imglist[i], int(cls_name_id)-1))
        return imglb_list
